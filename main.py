...
def count_pizza(video_path, region_points):
    cap = cv2.VideoCapture(video_path)
    assert cap.isOpened(), "Error reading video file"

    # Video writer
    w, h, fps = (int(cap.get(x)) for x in (cv2.CAP_PROP_FRAME_WIDTH, cv2.CAP_PROP_FRAME_HEIGHT, cv2.CAP_PROP_FPS))
    video_writer = cv2.VideoWriter("pizza_counting_output.avi", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h))
...
index = 0 # index is from 0-5
count_pizza(VIDEO_PATH[index], REGION_POINTS[index])